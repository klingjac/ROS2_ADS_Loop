...
class ICM20948:
    def write(self, reg, value):
        ...
        # print(f"bus={self._bus}, addr = {self._addr}, reg = {reg}, val = {value}")
    ...
    def __init__(self, i2c_addr=I2C_ADDR_ALT, i2c_bus=1):
        self._bank = -1
        self._addr = i2c_addr
        ...
        if not self.read(ICM20948_WHO_AM_I) == CHIP_ID:
            print(f"Test: {self.read(ICM20948_WHO_AM_I)}, {type(self.read(AK09916_WIA))} {int(self.read(ICM20948_WHO_AM_I)) == CHIP_ID}")
        ...
        if not self.mag_read(AK09916_WIA) == AK09916_CHIP_ID:
            print(f"Test: {self.mag_read(AK09916_WIA)}, {type(self.mag_read(AK09916_WIA))} {int(self.mag_read(AK09916_WIA)) == AK09916_CHIP_ID}")
