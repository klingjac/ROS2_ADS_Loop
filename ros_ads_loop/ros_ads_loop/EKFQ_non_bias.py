...
class QuaternionKalmanFilter:
    ...
    dt = 1/37  # Time step
    Q = np.eye(7) * 0.1  # Process noise covariance (example)
    R = np.eye(7) * 0.1
    state = np.array([1, 0, 0, 0, 0, 0, 0], dtype=np.float64)  # Initial quaternion (w, x, y, z)
    P = np.eye(7)  # Initial state covariance

    def predict(self, gyro_meas):
        
        #Body frame gyro measurements to a quaternion
        omega = np.hstack((gyro_meas))
    ...
    def compute_rate(q, r):
        
        w0, x0, y0, z0, bx, by, bz = q
        w1, x1, y1, z1 = r
        return np.array([
